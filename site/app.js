...
function cleanupCall(reason='',notify=false){const previous=callState;if(notify&&previous?.peerId)sendSignal({type:'hangup',callId:previous.callId},previous.peerId);clearCallTimers();stopLocalStream();callState=null;incomingInvite=null;resetCallUi();if(reason)toast(reason)}
async function acquireMedia(mode){if(!navigator.mediaDevices?.getUserMedia)throw new Error('Voice and video calls require HTTPS and a supported browser.');return navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true},video:mode==='video'?{facingMode:'user',width:{ideal:1280},height:{ideal:720}}:false})}
...
function receiveInvite(data,peerId){
  ...
  incomingInvite={callId:data.callId,peerId,mode:data.mode};setCallPerson('incoming',peerId);$('incomingType').textContent=data.mode==='video'?'Incoming video call':'Incoming voice call';$('incomingModal').classList.remove('hidden');
  callTimeout=setTimeout(()=>{if(incomingInvite?.callId===data.callId){sendSignal({type:'decline',callId:data.callId},peerId);incomingInvite=null;resetCallUi()}},30000)
}
async function acceptIncoming(){if(!incomingInvite)return;clearTimeout(callTimeout);callState={...incomingInvite,direction:'incoming',startedAt:null};incomingInvite=null;sendSignal({type:'accept',callId:callState.callId,mode:callState.mode},callState.peerId);await beginConnectedCall()}
function declineIncoming(){if(!incomingInvite)return;sendSignal({type:'decline',callId:incomingInvite.callId},incomingInvite.peerId);incomingInvite=null;clearCallTimers();resetCallUi()}
async function handleCallSignal(data,peerId){
  ...
  if(incomingInvite?.callId===data.callId&&['cancel','hangup'].includes(data.type)){incomingInvite=null;clearCallTimers();resetCallUi();return}
...
function handlePeerStream(stream,peerId,metadata){if(!callState||peerId!==callState.peerId||metadata?.kind!=='ephemera-call'||metadata?.callId!==callState.callId)return;remoteStream=stream;const video=$('remoteVideo');video.srcObject=stream;const play=()=>video.play().then(()=>$('mediaNote').classList.add('hidden')).catch(()=>$('mediaNote').classList.remove('hidden'));play();video.onclick=play}
...
function setup(desc){
  ...
  room.onPeerStream=handlePeerStream;room.onPeerJoin=id=>{peers.add(id);renderPeople();addSystem(user(id).name+' joined')};room.onPeerLeave=id=>{peers.delete(id);renderPeople();addSystem(user(id).name+' left');if(callState?.peerId===id)cleanupCall('The other participant left the room.');if(incomingInvite?.peerId===id){incomingInvite=null;resetCallUi()}};$('status').textContent='Connected · encrypted peer-to-peer'
}
...
$('text').oninput=()=>{sendTyping?.({on:true});clearTimeout(typingTimer);typingTimer=setTimeout(()=>sendTyping?.({on:false}),1200)};$('text').onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();$('send').click()}};$('emoji').onclick=()=>{$('text').value+=' 😊';$('text').focus()};$('fileBtn').onclick=()=>$('file').click();
...
$('copy').onclick=async()=>{try{await navigator.clipboard.writeText(location.href);toast('Room link copied')}catch{toast('Copy the room URL from your address bar.')}};$('clear').onclick=()=>{messages.innerHTML='';messages.append(empty);empty.classList.remove('hidden');msgs.clear()};$('cancelReply').onclick=()=>{reply=null;$('replybar').classList.add('hidden')};$('peopleToggle').onclick=()=>peoplePanel.classList.toggle('open');$('leave').onclick=()=>{cleanupCall('',true);room?.leave();room=null;history.replaceState(null,'',location.pathname);roomEl.classList.add('hidden');landing.classList.remove('hidden')};
...
addEventListener('hashchange',()=>{const p=parse();if(p)setup(p)});addEventListener('beforeunload',()=>{cleanupCall('',true);room?.leave()});const initial=parse();if(initial)setup(initial);
